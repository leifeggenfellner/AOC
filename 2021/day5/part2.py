...
for coord_set in coords:
    if coord_set[0][0] != coord_set[1][0] and coord_set[0][1] != coord_set[1][1]:
        if coord_set[0][1] < coord_set[1][1]:
            rows = [i for i in range(coord_set[0][1], coord_set[1][1] + 1)]
        else:
            rows = [i for i in range(coord_set[0][1], coord_set[1][1] - 1, -1)]
        if coord_set[0][0] < coord_set[1][0]:
            cols = [i for i in range(coord_set[0][0], coord_set[1][0] + 1)]
        else:
            cols = [i for i in range(coord_set[0][0], coord_set[1][0] - 1, -1)]
